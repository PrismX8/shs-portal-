...
let counterPollInterval = null;
  ...
  function cleanupOnUnload() {
      try {
          ...
          // Clean up Firebase online presence
          if (db && visitorId) {
              try {
                  db.ref('online/' + visitorId).remove();
                  db.ref('chatTyping/' + visitorId).remove();
              } catch (error) {
                  console.error('Error cleaning up online presence:', error);
              }
          } else if (backendApi && chatBackendReady) {
  ...
  onFirebaseReady(() => {
    ...
    if (typeof onlineRef.onDisconnect === 'function') {
...
// Change Name/Color popup
  ...
  (function enhanceContextMenu() {
      ...
      allGamesBtn?.addEventListener('click', () => {
          let allGamesPath = 'pages/all-games.html';
          if (path.includes('/games/')) {
              allGamesPath = '../pages/all-games.html';
          }
...
closeFullscreenChatBtn?.addEventListener('click', () => {
  ...
  async function sendChatMessage(inputElement) {
    ...
    // Check if user is banned
    const banned = await isUserBannedAsync(visitorId);
    if (banned) {
        if (typeof notifications !== 'undefined' && notifications.show) {
  ...
  function updateAchievements() {
    ...
    if (userStats.chatMessages >= 10) {
      ...
      if (chatBadgeEl) {
        chatBadgeEl.style.borderColor = '#28a745';
        chatBadgeEl.style.background = '#d4edda';
      }
    }
    ...
    if (userStats.drawTime >= 30) {
      ...
      if (drawBadgeEl) {
        drawBadgeEl.style.borderColor = '#28a745';
        drawBadgeEl.style.background = '#d4edda';
      }
    }
    ...
    if (userStats.gameTime >= 60) {
      ...
      if (gameBadgeEl) {
        gameBadgeEl.style.borderColor = '#28a745';
        gameBadgeEl.style.background = '#d4edda';
      }
    }
    ...
    if (userStats.onlineTime >= 120) {
      ...
      if (socialBadgeEl) {
        socialBadgeEl.style.borderColor = '#28a745';
        socialBadgeEl.style.background = '#d4edda';
      }
    }
  ...
  function renderFeaturedSections() {
      ...
      sections.push(
          ...
          },
          {
              ...
              category: 'popular',
              link: 'pages/all-games.html?category=popular'
          },
          {
