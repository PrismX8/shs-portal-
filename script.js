...
// Update existing button handlers to use page switching
document.addEventListener('DOMContentLoaded', () => {
    // Override profile button
    const profileBtn = document.getElementById('profileBtn');
    if (profileBtn) {
        const originalHandler = profileBtn.onclick;
        profileBtn.addEventListener('click', (e) => {
            e.preventDefault();
            switchPage('profile');
            // Also trigger original profile modal if it exists
            const profileModal = document.getElementById('profileModalOverlay') || document.getElementById('profileModal');
            if (profileModal && typeof originalHandler === 'function') {
                setTimeout(() => originalHandler(e), 100);
            }
        });
    }
    
    // Override leaderboard button
    const leaderboardBtn = document.getElementById('leaderboardBtn');
    if (leaderboardBtn) {
        leaderboardBtn.addEventListener('click', (e) => {
            ...
            if (leaderboardModal) {
                ...
                leaderboardModal.style.position = 'relative';
                leaderboardModal.style.background = 'transparent';
                leaderboardModal.style.boxShadow = 'none';
            }
        });
    }
    
    // Override friends button
    const friendsBtn = document.getElementById('friendsBtn');
    if (friendsBtn) {
        friendsBtn.addEventListener('click', (e) => {
            ...
            if (friendsModal) {
                friendsModal.style.display = 'block';
                friendsModal.style.position = 'relative';
                friendsModal.style.background = 'transparent';
                friendsModal.style.boxShadow = 'none';
            }
        });
    }
    
    // Override drawing button
    const openDrawingBtn = document.getElementById('openDrawingBtn');
    if (openDrawingBtn) {
        openDrawingBtn.addEventListener('click', (e) => {
            e.preventDefault();
            switchPage('drawing');
            // Show drawing modal
            const drawingModal = document.getElementById('drawingModal');
            if (drawingModal) {
                ...
                drawingModal.style.position = 'relative';
            }
        });
    }
